...
from read_ptv_is_files import read_ptv_is_files
# import math as m
# import pdb
...
#        print filename
#        frame = np.atleast_1d(np.genfromtxt(filename,dtype="i4,i4,f8,f8,f8",
...
#------------------------------
def _build_trajectories_lines(data):
    """
    build_trajectories_lines(data) is responsible for the book keeping of the trajectories, 
    using the prev,next fields in the frames,creating new set of data, in the form of trajectories
    """
    # first frame, initialization
    ...
    for i,frame in enumerate(data[1:]):
        for p in frame:
            if p['prev'] > -1:
                p['trajid'] = data[i]['trajid'][p['prev']-1]
            elif p['prev'] == -1 and p['next'] != -2:
                p['trajid'] += trajid
...
#----------------------------
def _ptv_to_traj(data,minlength=3):
    """ 
...
if __name__ == '__main__':
    print(' Reading ... \n')
    directory = '../test_data' 
    data = read_ptv_is_files(directory,10000,10004) 
    print(' Building trajectories ... \n')
    tracked_data = _build_trajectories_lines(data)
    np.save('tracked_data',tracked_data)
    print('Restructuring and deriving velocity ... \n')
    traj = _ptv_to_traj(tracked_data)
    print(' Saving ... \n')
    np.save('traj',traj)
# 	
