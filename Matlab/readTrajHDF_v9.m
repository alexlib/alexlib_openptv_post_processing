function [dstr, astr] = readTrajHDF(hfile,varargin)
% Full release, ver. 9
%
% [TRAJECTORIES,ATTRIBUTES] = READTRAJHDF(HDF_FILE,FIELD,VALUE,FIELD,VALUE)
%
% ReadTrajHDF reads the particle and flow quantities along the trajectories 
% from the HDF files (HDF version release 4.2, new Vdata format, January
% 2004)
% 
% Inputs:
...
%
% Example: 
%           [traj,attr] = readTrajHDF('polymer.hdf','u',[],'wws',[],'minlength',20,'frames',5);
%
% See also: HELP TRAJPOINT2HDF
%

% Release version 8.0, April 17, 2004
% Last modified in _v8.m file
%
% Last modified: December 1, 2003
% on ihw-liberzon
% to be able read VS structure instead of SD 
% Last modified: Dec. 5, 2003
% on ihw-liberzon
% - the option to have an empty structure, if not even one single trajectory
% will be completed
% - the bug of the length of the vrefs is solved
% 
% version 5, March 25
% - new version due to the changes in 
% the trajPoint2HDF_v8.m
% - one HDF file
% - one object per file
% - instead of startP (t) column now it is traj
% - instead of age now it is t column
% Full release version: readTrajHDF_v5.m, ETH PC computer
% - modified version 6
% - add the conditioning on other quantities, besides trajectory number
% After release version 8
% - long time reading of the water.hdf file
% - implemented reading of small number of frames
