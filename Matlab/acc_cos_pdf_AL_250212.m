% function res=acc_cos_pdf_AL_250212(first,last,tail,stepSize,threshold);
...
matfile = '0104_water_acc_cos_pdf.mat';
...
hist_absa=zeros(nPoints,nCols);
hist_absao=zeros(nPoints,nCols);
hist_absap=zeros(nPoints,nCols);
hist_absal=zeros(nPoints,nCols);
hist_absac=zeros(nPoints,nCols);
% hist_absus=zeros(nPoints,nCols);
% hist_absuo=zeros(nPoints,nCols);
hist_curv=zeros(nPoints,nCols);
hist_curvac=zeros(nPoints,nCols);

% cos_alac=zeros(nPoints,nCols);
% cos_usac=zeros(nPoints,nCols);
% cos_uoac=zeros(nPoints,nCols);
% cos_usuo=zeros(nPoints,nCols);
% cos_apac=zeros(nPoints,nCols);
% cos_apus=zeros(nPoints,nCols);
% cos_aouo=zeros(nPoints,nCols);

cos_a_w=zeros(nPoints,nCols);
% cos_ao_w=zeros(nPoints,nCols);
% cos_ap_w=zeros(nPoints,nCols);
cos_ac_w=zeros(nPoints,nCols);
% cos_al_w=zeros(nPoints,nCols);
% cos_us_w=zeros(nPoints,nCols);

cos_a_l1=zeros(nPoints,nCols);
% cos_ao_l1=zeros(nPoints,nCols);
% cos_ap_l1=zeros(nPoints,nCols);
cos_ac_l1=zeros(nPoints,nCols);
% cos_al_l1=zeros(nPoints,nCols);
% cos_us_l1=zeros(nPoints,nCols);
% cos_uo_l1=zeros(nPoints,nCols);

cos_a_l2=zeros(nPoints,nCols);
% cos_ao_l2=zeros(nPoints,nCols);
% cos_ap_l2=zeros(nPoints,nCols);
cos_ac_l2=zeros(nPoints,nCols);
% cos_al_l2=zeros(nPoints,nCols);
% cos_us_l2=zeros(nPoints,nCols);
% cos_uo_l2=zeros(nPoints,nCols);

cos_a_l3=zeros(nPoints,nCols);
% cos_ao_l3=zeros(nPoints,nCols);
% cos_ap_l3=zeros(nPoints,nCols);
cos_ac_l3=zeros(nPoints,nCols);
% cos_al_l3=zeros(nPoints,nCols);
% cos_us_l3=zeros(nPoints,nCols);
% cos_uo_l3=zeros(nPoints,nCols);

% cos_a_u=zeros(nPoints,nCols);
% cos_ao_u=zeros(nPoints,nCols);
% cos_ap_u=zeros(nPoints,nCols);
% cos_ac_u=zeros(nPoints,nCols);
% cos_al_u=zeros(nPoints,nCols);
% cos_us_u=zeros(nPoints,nCols);
            ...
            for ii=start:ende
                ...
                if age(ii)>tail && age(ii)<leng-tail+1 && reldiv(ii)<0.2 && absa(ii)^2/8e-4<100 && curv(ii)<3000
                    ...
                    if 2*strain>enstro(ii) && 2*strain>10^threshold %& ua(ii)<0
                        if count_strain<nPoints+1
                            count_strain=count_strain+1;
                            hist_absa(count_strain,nCols)=absa(ii);
                            %                                 hist_absao(count_strain,nCols)=absaO(ii);
                            %                                 hist_absap(count_strain,nCols)=absaP(ii);
                            %                                 hist_absal(count_strain,nCols)=absal(ii);
                            %                                 hist_absac(count_strain,nCols)=absac(ii);
                            hist_curv(count_strain,nCols)=curv(ii);
                            hist_curvac(count_strain,nCols)=curvac(ii);
                            %                                 hist_absus(count_strain,nCols)=absus(ii);
                            %                                 hist_absuo(count_strain,nCols)=absuo(ii);
                            
                            cos_a_w(count_strain,nCols)=[ax(ii) ay(ii) az(ii)]*[w1(ii) w2(ii) w3(ii)]'/(absa(ii)*absw(ii));
                            cos_ao_w(count_strain,nCols)=[aOx(ii) aOy(ii) aOz(ii)]*[w1(ii) w2(ii) w3(ii)]'/(absaO(ii)*absw(ii));
                            cos_ap_w(count_strain,nCols)=[aPx(ii) aPy(ii) aPz(ii)]*[w1(ii) w2(ii) w3(ii)]'/(absaP(ii)*absw(ii));
                            cos_ac_w(count_strain,nCols)=[acx(ii) acy(ii) acz(ii)]*[w1(ii) w2(ii) w3(ii)]'/(absac(ii)*absw(ii));
                            %                                 cos_al_w(count_strain,nCols)=[alx(ii) aly(ii) alz(ii)]*[w1(ii) w2(ii) w3(ii)]'/(absal(ii)*absw(ii));
                            %                                 cos_us_w(count_strain,nCols)=[usx(ii) usy(ii) usz(ii)]*[w1(ii) w2(ii) w3(ii)]'/(absus(ii)*absw(ii));
                            %                                 cos_uo_w(count_strain,nCols)=[uox(ii) uoy(ii) uoz(ii)]*[w1(ii) w2(ii) w3(ii)]'/(absuo(ii)*absw(ii));
                            
                            %                                 cos_a_l1(count_strain,nCols)=[ax(ii) ay(ii) az(ii)]*V(:,1)/(absa(ii)*(sum(V(:,1).^2))^0.5);
                            %                                 cos_ao_l1(count_strain,nCols)=[aOx(ii) aOy(ii) aOz(ii)]*V(:,1)/(absaO(ii)*(sum(V(:,1).^2))^0.5);
                            %                                 cos_ap_l1(count_strain,nCols)=[aPx(ii) aPy(ii) aPz(ii)]*V(:,1)/(absaP(ii)*(sum(V(:,1).^2))^0.5);
                            %                                 cos_ac_l1(count_strain,nCols)=[acx(ii) acy(ii) acz(ii)]*V(:,1)/(absac(ii)*(sum(V(:,1).^2))^0.5);
                            %                                 cos_al_l1(count_strain,nCols)=[alx(ii) aly(ii) alz(ii)]*V(:,1)/(absal(ii)*(sum(V(:,1).^2))^0.5);
                            %                                 cos_us_l1(count_strain,nCols)=[usx(ii) usy(ii) usz(ii)]*V(:,1)/(absus(ii)*(sum(V(:,1).^2))^0.5);
                            %                                 cos_uo_l1(count_strain,nCols)=[uox(ii) uoy(ii) uoz(ii)]*V(:,1)/(absuo(ii)*(sum(V(:,1).^2))^0.5);
                            %
                            %                                 cos_a_l2(count_strain,nCols)=[ax(ii) ay(ii) az(ii)]*V(:,2)/(absa(ii)*(sum(V(:,2).^2))^0.5);
                            %                                 cos_ao_l2(count_strain,nCols)=[aOx(ii) aOy(ii) aOz(ii)]*V(:,2)/(absaO(ii)*(sum(V(:,2).^2))^0.5);
                            %                                 cos_ap_l2(count_strain,nCols)=[aPx(ii) aPy(ii) aPz(ii)]*V(:,2)/(absaP(ii)*(sum(V(:,2).^2))^0.5);
                            %                                 cos_ac_l2(count_strain,nCols)=[acx(ii) acy(ii) acz(ii)]*V(:,2)/(absac(ii)*(sum(V(:,2).^2))^0.5);
                            %                                 cos_al_l2(count_strain,nCols)=[alx(ii) aly(ii) alz(ii)]*V(:,2)/(absal(ii)*(sum(V(:,2).^2))^0.5);
                            %                                 cos_us_l2(count_strain,nCols)=[usx(ii) usy(ii) usz(ii)]*V(:,2)/(absus(ii)*(sum(V(:,2).^2))^0.5);
                            %                                 cos_uo_l2(count_strain,nCols)=[uox(ii) uoy(ii) uoz(ii)]*V(:,2)/(absuo(ii)*(sum(V(:,2).^2))^0.5);
                            %
                            %                                 cos_a_l3(count_strain,nCols)=[ax(ii) ay(ii) az(ii)]*V(:,3)/(absa(ii)*(sum(V(:,3).^2))^0.5);
                            %                                 cos_ao_l3(count_strain,nCols)=[aOx(ii) aOy(ii) aOz(ii)]*V(:,3)/(absaO(ii)*(sum(V(:,3).^2))^0.5);
                            %                                 cos_ap_l3(count_strain,nCols)=[aPx(ii) aPy(ii) aPz(ii)]*V(:,3)/(absaP(ii)*(sum(V(:,3).^2))^0.5);
                            %                                 cos_ac_l3(count_strain,nCols)=[acx(ii) acy(ii) acz(ii)]*V(:,3)/(absac(ii)*(sum(V(:,3).^2))^0.5);
                            %                                 cos_al_l3(count_strain,nCols)=[alx(ii) aly(ii) alz(ii)]*V(:,3)/(absal(ii)*(sum(V(:,3).^2))^0.5);
                            %                                 cos_us_l3(count_strain,nCols)=[usx(ii) usy(ii) usz(ii)]*V(:,3)/(absus(ii)*(sum(V(:,3).^2))^0.5);
                            %                                 cos_uo_l3(count_strain,nCols)=[uox(ii) uoy(ii) uoz(ii)]*V(:,3)/(absuo(ii)*(sum(V(:,3).^2))^0.5);
                            %
                            %                                 cos_a_u(count_strain,nCols)=[ax(ii) ay(ii) az(ii)]*[u(ii) v(ii) w(ii)]'/(absa(ii)*absu(ii));
                            %                                 cos_ao_u(count_strain,nCols)=[aOx(ii) aOy(ii) aOz(ii)]*[u(ii) v(ii) w(ii)]'/(absaO(ii)*absu(ii));
                            %                                 cos_ap_u(count_strain,nCols)=[aPx(ii) aPy(ii) aPz(ii)]*[u(ii) v(ii) w(ii)]'/(absaP(ii)*absu(ii));
                            %                                 cos_ac_u(count_strain,nCols)=[acx(ii) acy(ii) acz(ii)]*[u(ii) v(ii) w(ii)]'/(absac(ii)*absu(ii));
                            %                                 cos_al_u(count_strain,nCols)=[alx(ii) aly(ii) alz(ii)]*[u(ii) v(ii) w(ii)]'/(absal(ii)*absu(ii));
                            %                                 cos_us_u(count_strain,nCols)=[usx(ii) usy(ii) usz(ii)]*[u(ii) v(ii) w(ii)]'/(absus(ii)*absu(ii));
                            %                                 cos_uo_u(count_strain,nCols)=[uox(ii) uoy(ii) uoz(ii)]*[u(ii) v(ii) w(ii)]'/(absuo(ii)*absu(ii));
                            %
                            %                                 cos_alac(count_strain,nCols)=[alx(ii) aly(ii) alz(ii)]*[acx(ii) acy(ii) acz(ii)]'/(absal(ii)*absac(ii));
                            %                                 cos_usac(count_strain,nCols)=[usx(ii) usy(ii) usz(ii)]*[acx(ii) acy(ii) acz(ii)]'/(absus(ii)*absac(ii));
                            %                                 cos_uoac(count_strain,nCols)=[uox(ii) uoy(ii) uoz(ii)]*[acx(ii) acy(ii) acz(ii)]'/(absuo(ii)*absac(ii));
                            %                                 cos_usuo(count_strain,nCols)=[usx(ii) usy(ii) usz(ii)]*[uox(ii) uoy(ii) uoz(ii)]'/(absus(ii)*absuo(ii));
                            %                                 cos_apac(count_strain,nCols)=[aPx(ii) aPy(ii) aPz(ii)]*[acx(ii) acy(ii) acz(ii)]'/(absaP(ii)*absac(ii));
                            %                                 cos_apus(count_strain,nCols)=[aPx(ii) aPy(ii) aPz(ii)]*[usx(ii) usy(ii) usz(ii)]'/(absaP(ii)*absus(ii));
                            %                                 cos_aouo(count_strain,nCols)=[aOx(ii) aOy(ii) aOz(ii)]*[uox(ii) uoy(ii) uoz(ii)]'/(absaO(ii)*absuo(ii));
                        end
                    end
                end
            end
...
grid on;
box on;
[n,x]=nhist(hist_absa(1:counter,1),100);
plot(x,n,'k');
[n,x]=nhist(hist_absa(1:count_low,2),100);
plot(x,n,'r');
[n,x]=nhist(hist_absa(1:count_enstro,3),100);
plot(x,n,'g');
[n,x]=nhist(hist_absa(1:count_strain,nCols),100);
plot(x,n,'b');
...
plot(x,n,'b');
xlabel('|a|,||,\perp,a_l,a_c','FontSize',16,'FontName','Times New Roman')
...
% plot(x,n,'g');
% [n,x]=nhist(hist_absao(1:count_strain,nCols),1000);
% plot(x,n,'b');
...
% plot(x,n,'g');
% [n,x]=nhist(hist_absap(1:count_strain,nCols),1000);
% plot(x,n,'b');
...
% plot(x,n,'g');
% [n,x]=nhist(hist_absal(1:count_strain,nCols),1000);
% plot(x,n,'b');
...
plot(x,n,'g');
[n,x]=nhist(hist_curv(1:count_strain,nCols),1000);
plot(x,n,'b');
xlabel('curvature','FontSize',16,'FontName','Times New Roman')
ylabel('pdf','FontSize',16,'FontName','Times New Roman')
set(gca,'XScale','log')
set(gca,'YScale','log')
...
plot(x,n,'k');
[n,x]=nhist(hist_curvac(1:count_low,2),1000);
plot(x,n,'r');
[n,x]=nhist(hist_curvac(1:count_enstro,3),1000);
plot(x,n,'g');
[n,x]=nhist(hist_curvac(1:count_strain,nCols),1000);
plot(x,n,'b');
xlabel('curvature a_c','FontSize',16,'FontName','Times New Roman')
ylabel('pdf','FontSize',16,'FontName','Times New Roman')
set(gca,'XScale','log')
set(gca,'YScale','log')
...
% plot(x,n,'g');
% [n,x]=nhist(hist_absus(1:count_strain,nCols),1000);
% plot(x,n,'b');
...
% plot(x,n,'g');
% [n,x]=nhist(hist_absuo(1:count_strain,nCols),1000);
% plot(x,n,'b');
...
[n,x]=nhist(cos_ac_w(1:counter,1),50);
plot(x,n,'g');
[n,x]=nhist(cos_a_w(1:count_strain,nCols),50);
plot(x,n,'k--');
[n,x]=nhist(cos_ao_w(1:count_strain,nCols),50);
plot(x,n,'r--');
[n,x]=nhist(cos_ap_w(1:count_strain,nCols),50);
plot(x,n,'c--');
[n,x]=nhist(cos_ac_w(1:count_strain,nCols),50);
plot(x,n,'g--');
xlabel('cos(a,\lambda_2)','FontSize',12,'FontName','Times New Roman')
ylabel('pdf','FontSize',12,'FontName','Times New Roman')
axis([-1 1 0 2])
...
% [n,x]=nhist(cos_ap_w(1:count_strain,nCols),50);
% plot(x,n,'g');
% [n,x]=nhist(cos_al_w(1:count_strain,nCols),50);
% plot(x,n,'b');
% [n,x]=nhist(cos_ac_w(1:count_strain,nCols),50);
% plot(x,n,'c');
% [n,x]=nhist(cos_us_w(1:count_strain,nCols),50);
% plot(x,n,'m');
...
% grid on;
% box on;
% [n,x]=nhist(cos_a_u(1:count_strain,nCols),50);
% plot(x,n,'k');
% [n,x]=nhist(cos_al_u(1:count_strain,nCols),50);
% plot(x,n,'b');
% [n,x]=nhist(cos_ac_u(1:count_strain,nCols),50);
% plot(x,n,'c');
% [n,x]=nhist(cos_us_u(1:count_strain,nCols),50);
% plot(x,n,'m');
...
% grid on;
% box on;
% [n,x]=nhist(cos_a_l1(1:count_strain,nCols),50);
% plot(x,n,'k');
% [n,x]=nhist(cos_ao_l1(1:count_strain,nCols),50);
% plot(x,n,'r');
% [n,x]=nhist(cos_ap_l1(1:count_strain,nCols),50);
% plot(x,n,'g');
% [n,x]=nhist(cos_al_l1(1:count_strain,nCols),50);
% plot(x,n,'b');
% [n,x]=nhist(cos_ac_l1(1:count_strain,nCols),50);
% plot(x,n,'c');
% [n,x]=nhist(cos_us_l1(1:count_strain,nCols),50);
% plot(x,n,'m');
% [n,x]=nhist(cos_uo_l1(1:count_strain,nCols),50);
% plot(x,n,'y');
...
% grid on;
% box on;
% [n,x]=nhist(cos_a_l2(1:count_strain,nCols),50);
% plot(x,n,'k');
% [n,x]=nhist(cos_ao_l2(1:count_strain,nCols),50);
% plot(x,n,'r');
% [n,x]=nhist(cos_ap_l2(1:count_strain,nCols),50);
% plot(x,n,'g');
% [n,x]=nhist(cos_al_l2(1:count_strain,nCols),50);
% plot(x,n,'b');
% [n,x]=nhist(cos_ac_l2(1:count_strain,nCols),50);
% plot(x,n,'c');
% [n,x]=nhist(cos_us_l2(1:count_strain,nCols),50);
% plot(x,n,'m');
% [n,x]=nhist(cos_uo_l2(1:count_strain,nCols),50);
% plot(x,n,'y');
...
% grid on;
% box on;
% [n,x]=nhist(cos_a_l3(1:count_strain,nCols),50);
% plot(x,n,'k');
% [n,x]=nhist(cos_ao_l3(1:count_strain,nCols),50);
% plot(x,n,'r');
% [n,x]=nhist(cos_ap_l3(1:count_strain,nCols),50);
% plot(x,n,'g');
% [n,x]=nhist(cos_al_l3(1:count_strain,nCols),50);
% plot(x,n,'b');
% [n,x]=nhist(cos_ac_l3(1:count_strain,nCols),50);
% plot(x,n,'c');
% [n,x]=nhist(cos_us_l3(1:count_strain,nCols),50);
% plot(x,n,'m');
% [n,x]=nhist(cos_uo_l3(1:count_strain,nCols),50);
% plot(x,n,'y');
...
% plot(x,n,'g');
% [n,x]=nhist(cos_alac(1:count_strain,nCols),50);
% plot(x,n,'b');
...
% plot(x,n,'g');
% [n,x]=nhist(cos_usac(1:count_strain,nCols),50);
% plot(x,n,'b');
...
% plot(x,n,'g');
% [n,x]=nhist(cos_uoac(1:count_strain,nCols),50);
% plot(x,n,'b');
...
% plot(x,n,'g');
% [n,x]=nhist(cos_usuo(1:count_strain,nCols),50);
% plot(x,n,'b');
...
% plot(x,n,'g');
% [n,x]=nhist(cos_apac(1:count_strain,nCols),50);
% plot(x,n,'b');
...
% plot(x,n,'g');
% [n,x]=nhist(cos_apus(1:count_strain,nCols),50);
% plot(x,n,'b');
...
% plot(x,n,'g');
% [n,x]=nhist(cos_aouo(1:count_strain,nCols),50);
% plot(x,n,'b');
