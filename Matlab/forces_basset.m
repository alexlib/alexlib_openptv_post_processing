...
if ismac
    matdirectory = '\Users\alex\Dropbox\resuspension\Matlab\trajectories_alex_2012(186-194)\events_110';
else
...
[   time_data, time_of_event, drag_Urev, drag_force,  norm_velocity_f_p1,...
    inertia_force, lift_force, pressure_force, data_acceleration_p, ...
    total_force, buoyancy_force_1, basset_force1,inertia_force_part1, ...
    data_velocity_f_p1, data_velocity_p_f1,data_velocity_f1, norm_velocity_f_p, ...
    data_acceleration_f1, data_acceleration_p_f1,inertia_force_part2,...
   ...
   for iEvents=1:size(total_force,2);  
    if ~isempty([time_of_event{kFiles,iEvents}]);
       figure, hold on, 
        ...
        plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}],norm_total_force,'b');
        plot([time_data{kFiles,iEvents,1:end}]-[time_data{kFiles,iEvents,6}],norm_basset_force1,'r');
        plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}],norm_inertia,'g');

        plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}],norm_pressure,'m');
        plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}], norm_buoyancy,'c');
        
        plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}], norm_drag,'y');
        plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}], norm_lift,'c');
...
%         
        plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}],vertical_total_force,'b');
        ...
        plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}], vertical_drag,'y');
        plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}], vertical_lift,'c');
   ...
   for iEvents=1:size(total_force,2);  
    if ~isempty([time_of_event{kFiles,iEvents}]);
       figure, hold on, 
        ...
        plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}],horizontal_total_force,'b');
        plot([time_data{kFiles,iEvents,1:end}]-[time_data{kFiles,iEvents,6}],horizontal_basset_force1,'r');
        plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}],horizontal_inertia,'g');
...
%         plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}],horizontal_inertia_part2,'c');
        plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}], horizontal_pressure,'m');
        plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}], horizontal_drag,'y');
        plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}], horizontal_lift,'c');
...
for kFiles = 1:size(total_force,1)
    for iEvents = 1:size(total_force,2)
        if ~isempty([time_of_event{kFiles,iEvents}]);
            tmp = cat(1,total_force{kFiles,iEvents,:});
            vertical_tmp = tmp(:,2);
            plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}],vertical_tmp);
            t1 = max([time_of_event{kFiles,iEvents}]-[time_data{kFiles,iEvents,1}]-1,1);
...
for kFiles = 1:size(total_force,1)
    for iEvents = 1:size(total_force,2)
        if ~isempty([time_of_event{kFiles,iEvents}]);
            tmp = cat(1,total_force{kFiles,iEvents,:});
            norm_horizontal_tmp = sum(tmp(:,[1 3]),2);
            plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}],norm_horizontal_tmp);
            t1 = max([time_of_event{kFiles,iEvents}]-[time_data{kFiles,iEvents,1}]-1,1);
...
%             tmp3 = cat(1,inertia_force{kFiles,iEvents,:});
%             vertical_inertia = tmp3(:,2);
%             plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}],vertical_inertia);
%             % tmp = [data_drag{kFiles,iEvents,:}];
%             t1 = max([time_of_event{kFiles,iEvents}]-[time_data{kFiles,iEvents,1}]-1,1);
...
%             tmp3 = cat(1,inertia_force{kFiles,iEvents,:});
%             horizontal_inertia = sum(tmp3(:,[1 3]).^2,2).^(0.5);
%             plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}],horizontal_inertia);
%             % tmp = [data_drag{kFiles,iEvents,:}];
%             t1 = max([time_of_event{kFiles,iEvents}]-[time_data{kFiles,iEvents,1}]-1,1);
...
%             tmp4 = cat(1,inertia_force_part1{kFiles,iEvents,:});
%             vertical_inertia_part1 = tmp4(:,2);
%             plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}],vertical_inertia_part1);
%             % tmp = [data_drag{kFiles,iEvents,:}];
%             t1 = max([time_of_event{kFiles,iEvents}]-[time_data{kFiles,iEvents,1}]-1,1);
...
%             tmp4 = cat(1,inertia_force_part1{kFiles,iEvents,:});
%             horizontal_inertia_part1 = sum(tmp4(:,[1 3]).^2,2).^(0.5);
%             plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}],horizontal_inertia_part1);
%             % tmp = [data_drag{kFiles,iEvents,:}];
%             t1 = max([time_of_event{kFiles,iEvents}]-[time_data{kFiles,iEvents,1}]-1,1);
...
%             tmp5 = cat(1,inertia_force_part2{kFiles,iEvents,:});
%             vertical_inertia_part2 = tmp5(:,2);
%             plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}],vertical_inertia_part2);
%             % tmp = [data_drag{kFiles,iEvents,:}];
%             t1 = max([time_of_event{kFiles,iEvents}]-[time_data{kFiles,iEvents,1}]-1,1);
...
%             tmp5 = cat(1,inertia_force_part2{kFiles,iEvents,:});
%             horizontal_inertia_part2 = sum(tmp5(:,[1 3]).^2,2).^(0.5);
%             plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}],horizontal_inertia_part2);
%             % tmp = [data_drag{kFiles,iEvents,:}];
%             t1 = max([time_of_event{kFiles,iEvents}]-[time_data{kFiles,iEvents,1}]-1,1);
...
%             tmp6 = cat(1,pressure_force{kFiles,iEvents,:});
%             vertical_pressure = tmp6(:,2);
%             plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}],vertical_pressure);
%             % tmp = [data_drag{kFiles,iEvents,:}];
%             t1 = max([time_of_event{kFiles,iEvents}]-[time_data{kFiles,iEvents,1}]-1,1);
...
%             plot([time_data{kFiles,iEvents,6:end}]-[time_data{kFiles,iEvents,6}],horizontal_pressure);
%             % tmp = [data_drag{kFiles,iEvents,:}];
%             t1 = max([time_of_event{kFiles,iEvents}]-[time_data{kFiles,iEvents,1}]-1,1);
