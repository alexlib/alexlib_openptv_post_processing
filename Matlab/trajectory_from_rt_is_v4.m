...
WF=1;
    ...
    %var=load([fold,num2str(WF),'/variables']);
    %tol=var.tol;
    %radius_sphere=var.radius_sphere;
    %parent=var.parent;
    %child=var.child;
    ff=[];
...
wd= 'E:\OLD-Data-DISK\Matlab for PTV';
cd([fold,num2str(WF)]);
save  variables_v2 WF wd fold fold2 xa ya za ta tx ty tz...
                ...
                rel_div  radius_sphere  first last -append 
...
%parent=[4 36 ];%parent traj ID
%child=[10]; % children traj id

save variables_v2 parent child -append
...
% In total 33 variables are stored

save parent_variables_v2  X Y Z T...
     parent_totalpix  parent_xpix  parent_ypix  parent_grv  parent ...
...
save children_variables_v2  cx cy cz...
...
trajLen=traj_len;
trajLen([parent child])=nan;
longTraj=find(trajLen>56);
...
for nb_id=1:length(non_break)%%% If in case parent trajectory is fragmented


    nb_t=time(non_break(nb_id),:);
    nb_index=find(nb_t>0);
    nb_T=[nb_T;nb_t(nb_index)'];
    nb_x=tx(non_break(nb_id),:);
    nb_X=[nb_X;nb_x(nb_index)'];%%% This is a bug remover !!
    nb_y=ty(non_break(nb_id),:);
    nb_Y=[nb_Y;nb_y(nb_index)'];
    nb_z=tz(non_break(nb_id),:);
    nb_Z=[nb_Z;nb_z(nb_index)'];
    
    nbtotalpix=totalpix(non_break(nb_id),:);
    nb_totalpix=[nb_totalpix;nbtotalpix(nb_index)'];
    
    nbxpix=xpix(non_break(nb_id),:);
    nb_xpix=[nb_xpix;nbxpix(nb_index)'];
    
    nbypix=ypix(non_break(nb_id),:);
    nb_ypix=[nb_ypix;nbypix(nb_index)'];
    
    nbgrv=grv(non_break(nb_id),:);
    nb_arent_grv=[nb_grv;nbgrv(nb_index)'];
    
    nbco=co(non_break(nb_id),:);
    nb_co=[nb_co;nbco(nb_index)'];
    
    nbAij=squeeze(Aij(non_break(nb_id),:,:,:));
    nb_Aij=[nb_Aij;nbAij(nb_index,:,:)];
    
    nb_rotation=squeeze(rot_M(non_break(nb_id),:,:,:));
    nb_rij=[nb_rij;nb_rotation(nb_index,:,:)];
    
    nb_strain_tensor=squeeze(A(non_break(nb_id),:,:,:));
    nb_Sij=[nb_Sij;nb_strain_tensor(nb_index,:,:)];
    
    nbw1=w1(non_break(nb_id),:);
    nb_w1=[nb_w1;nbw1(nb_index)'];
    
    nbw2=w2(non_break(nb_id),:);
    nb_w2=[nb_w2;nbw2(nb_index)'];
    
    nbw3=w3(non_break(nb_id),:);
    nb_w3=[nb_w3;nbw3(nb_index)'];
    
    nbe1=e1(non_break(nb_id),:);
    nb_e1=[nb_e1;nbe1(nb_index)'];
    
    nbe2=e2(non_break(nb_id),:);
    nb_e2=[nb_e2;nbe2(nb_index)'];
    
    nbe3=e3(non_break(nb_id),:);
    nb_e3=[nb_e3;nbe3(nb_index)'];
    
    nbv1=squeeze(v1(non_break(nb_id),:,:));
    nb_v1=[nb_v1;nbv1(nb_index,:)];
    
    nbv2=squeeze(v2(non_break(nb_id),:,:));
    nb_v2=[nb_v2;nbv2(nb_index,:)];
    
    nbv3=squeeze(v3(non_break(nb_id),:,:));
    nb_v3=[nb_v3;nbv3(nb_index,:)];
    
    nbstrain=strain(non_break(nb_id),:);
    nb_strain=[nb_strain;nbstrain(:,nb_index)'];
    
    nbenstro=enstro(non_break(nb_id),:);
    nb_enstro=[nb_enstro;nbenstro(:,nb_index)'];
    
    nbmn_shrt_ax=mn_shrt_ax(non_break(nb_id),:);
    nb_mn_shrt_ax=[nb_mn_shrt_ax;nbmn_shrt_ax(:,nb_index)'];
    
    nbmax_shrt_ax=max_shrt_ax(non_break(nb_id),:);
    nb_max_shrt_ax=[nb_max_shrt_ax;nbmax_shrt_ax(:,nb_index)'];
    
    nbmn_shrt_ax_mm=mn_shrt_ax_mm(non_break(nb_id),:);
    nb_mn_shrt_ax_mm=[nb_mn_shrt_ax_mm;nbmn_shrt_ax_mm(:,nb_index)'];
    
    nbmax_shrt_ax_mm=max_shrt_ax_mm(non_break(nb_id),:);
    nb_max_shrt_ax_mm=[nb_max_shrt_ax_mm;nbmax_shrt_ax_mm(:,nb_index)'];

    nbmn_lng_ax=mn_lng_ax(non_break(nb_id),:);
    nb_arent_mn_lng_ax=[nb_mn_lng_ax;nbmn_lng_ax(:,nb_index)'];
    
    nbmax_lng_ax=max_lng_ax(non_break(nb_id),:);
    nb_max_lng_ax=[nb_max_lng_ax;nbmax_lng_ax(:,nb_index)'];
    
    nbmn_lng_ax_mm=mn_lng_ax_mm(non_break(nb_id),:);
    nb_mn_lng_ax_mm=[nb_mn_lng_ax_mm;nbmn_lng_ax_mm(:,nb_index)'];
    
    nbmax_lng_ax_mm=max_lng_ax_mm(non_break(nb_id),:);
    nb_max_lng_ax_mm=[nb_max_lng_ax_mm;nbmax_lng_ax_mm(:,nb_index)'];

    nbrel_div=rel_div(non_break(nb_id),:);
    nb_rel_div=[nb_rel_div;nbrel_div(:,nb_index)'];

end


save nb_variables_2  nb_X nb_Y nb_Z nb_T...
     nb_totalpix  nb_xpix  nb_ypix  nb_grv  non_break ...
