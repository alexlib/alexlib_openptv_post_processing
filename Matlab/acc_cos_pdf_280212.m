function res=acc_cos_pdf_280212(first,last,tail,stepSize,threshold)
...
for i=first:stepSize:last
    ...
    f=load(name);
    s=size(f);
%     f = hdfread(hfile,sprintf('/%d',i), 'Fields', 'x,y,z,u,v,w,ax,ay,az,w1,w2,w3,s11,s12,s13,s22,s23,s33,ww1,ww2,ww3,wws,sss,R,Q,diss,div,trajnum,t,alx,aly,alz', 'FirstRecord',1);
%     s = length(f{1})
    if s(1,1)>0
        xr=f{1};
        yr=f{2};
...
plot(x,n,'g');
[n,x]=nhist(hist_absa(1:count_strain,4),100);
plot(x,n,'b');
...
plot(x,n,'g');
[n,x]=nhist(hist_curvac(1:count_strain,4),1000);
plot(x,n,'b');
