function varargout = Grid2HDF(hfile,varargin)
% Grid2HDF Reads Grid.* files from latest version of
% Beat Luethi PostPTV(tm) software into Matlab and saves them 
% with the HDF format.
%
%    Grid2HDF(HFILE,FILEPATH,FIRSTFILE,LASTFILE)
% Will read Grid.* files from Grid.FIRSTFILE to
% Grid.LASTFILE in the directory FILEPATH
%
% Example:
%           Grid2HDF('Conv_run90.hdf')
% or
%
%           Grid2HDF('conv_run90.hdf','e:\ptv\convection\14july2004\res\',10000,12000);
%
...
fieldname_list = 'x,y,z,u,v,w,dudx,dudy,dudz,dvdx,dvdy,dvdz,dwdx,dwdy,dwdz,dudt,dvdt,dwdt,ax,ay,az,daxdx,daxdy,daxdz,daydx,daydy,daydz,dazdx,dazdy,dazdz,quality,reldiv';
...
try
    ...
    astr.Comment    = sprintf('By Grid2HDF, ver.%d ', 1);
    astr.Directory  = cd;
    ...
    if nargin < 4
        ...
        [filename1, pathname] = uigetfile('Grid.*', 'Pick a FIRST trajectory file');
        ...
        cd(pathname)
        [filename2, pathname] = uigetfile('Grid.*', 'Pick a LAST trajectory file');
