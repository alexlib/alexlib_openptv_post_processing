...
if ismac
    ...
    matdirectory ='C:\Users\hadar\Dropbox\resuspension\2011\trajectories(186-194)\events_100';
end
...
%save kinetic_energy_R25_80.mat
...
for kFiles = 1:size(data_small,1)
    for iEvents = 1:size(data_small,2)
        if ~isempty([time_of_event{kFiles,iEvents}])
            plot([time_data{kFiles,iEvents,:}]-[time_data{kFiles,iEvents,1}],[data_small{kFiles,iEvents,:}]); % x = time, y= kinetic energy small
            ...
            plot([time_data{kFiles,iEvents,:}]-[time_data{kFiles,iEvents,1}],[data_large{kFiles,iEvents,:}]);
        end
    end
end
hold off
...
for kFiles = 1:size(data_small,1)
    for iEvents = 1:size(data_small,2)
        if ~isempty([time_of_event{6,iEvents}])
            plot([time_data{6,iEvents,:}]-[time_data{6,iEvents,1}],[data_small{6,iEvents,:}]); % x = time, y= kinetic energy small
            tmp = [data_small{6,iEvents,:}];
            t1 = max([time_of_event{6,iEvents}]-[time_data{6,iEvents,1}]-1,1);
            plot(t1,tmp(t1+1),'ro','MarkerSize',10); % time of the event
            plot([time_data{6,iEvents,:}]-[time_data{6,iEvents,1}],[data_large{6,iEvents,:}]);
            tmp1 = [data_large{6,iEvents,:}];
            plot(t1,tmp1(t1+1),'ro','MarkerSize',10); % time of the event
        end
    end
end
hold off
