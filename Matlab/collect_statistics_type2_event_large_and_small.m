function [collected_data] = collect_statistics_type2_event_large_and_small(filename,quantity1,quantity2, R)
% COLLECT_STATISTICS_TYPE2_EVENT_LARGE_AND_SMALL(MATFILENAME,LARGE_QUANTITY,SMALL_QUANTITY, R)
    ...
    for j = 1:numTraj
        ...
        if tf % i.e. if this small trajectory is also appearing at that time
            ...
            if distance <= R
                ...
                collected_data = cat(1,collected_data,[data.(quantity1)(ind),small(j).(quantity2)(loc)]);
            end
        end
    end
end
