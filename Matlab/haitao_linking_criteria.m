...
% [TRAJ,REMOVELIST] = HAITAO_LINKNG_CRITERIA(TRAJ,MAXDT,DISTTHRESH) builds containers of trajectories that can be
...
%                 predicted_z = interp1(traj(i).t,traj(i).zf,traj(i).t(end)+dt,'cubic','extrap');
            ...
            predicted_y = predicted_x(2); predicted_z = predicted_x(3); predicted_x = predicted_x(1);
...
%                 predicted_z1 = interp1(traj(j).t,traj(j).zf,traj(j).t(1)-dt,'cubic','extrap');
            ...
            predicted_y1 = predicted_x1(2); predicted_z1 = predicted_x1(3); predicted_x1 = predicted_x1(1);
...
%          pause
         ...
         traj(linkid) = link_trajectories_smoothn(combine_trajectories(traj(i),traj(linkid)));
