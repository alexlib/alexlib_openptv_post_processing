function [dstr, astr] = readGridHDF(hfile,varargin)
%
% [TRAJECTORIES,ATTRIBUTES] = READGridHDF(HDF_FILE,FIELD,VALUE,FIELD,VALUE)
%
% ReadGridHDF reads the particle and flow quantities along the trajectories 
...
%
% Example: 
%           [data,attr] = readGridHDF('test.hdf','u',[],'dudx',[],'daxdx',[],'frames',[1 5]);
%
...
for i = 1:size(fieldNames,1)
    dstr.(fieldNames{i}) = repmat(0, 1000, 1); % assume 500 values in each trajectory
end
dstr = repmat(dstr,1,min(count,diff(frames))); % assume 500 trajectories per frame
...
%             ind = intersect(ind, find(tmp(:,j) >= fields.(fieldNames{j})(1) & tmp(:,j) <= fields.(fieldNames{j})(2)));
            ind = ind & (tmp(:,j) >= fields.(fieldNames{j})(1) & tmp(:,j) <= fields.(fieldNames{j})(2));
...
%         dstr(k).trajLen  = lenTraj(ind(m));
        for j = 1:length(fieldNames)
            dstr(k).(fieldNames{j}) = tmp(ind,j);            
        end
        
%     end
    clear tmp data firstRecord lenRecord ind j
