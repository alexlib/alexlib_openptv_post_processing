...
for kFiles = 1:length(large_ones)  
    ...
    for iEvents = 1:numEvents
        ...
        for ind = 1:length(data.t)
            ...
            data_drag{kFiles,iEvents,ind} =  drag_force / counter; % average  of the   absolute drag force
...
for kFiles = 1:size(data_drag,1)
    for iEvents = 1:size(data_drag,2)
        if ~isempty([time_of_event{kFiles,iEvents}])
            for 1:size(data_drag{kFiles,iEvents,:},3)
            plot([time_data_small{kFiles,iEvents,:}]-[time_data_small{kFiles,iEvents,1}],mean([data_drag{kFiles,iEvents}])); 
