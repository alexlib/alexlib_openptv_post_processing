...
fieldname_list = 'b,pb,qb,rb,db,vb,w,pw,qw,rw,dw,vw,t,pt,qt,rt,dt,vt,l,lt,zeta,age';
dfields = {'b','pb','qb','rb','db','vb','w','pw','qw','rw','dw','vw','t','pt','qt','rt','dt','vt','l','lt','zeta','age'};
order = {9,1,1,1,3,9,9,1,1,1,3,9,9,1,1,1,3,9,N*3,N*3,N,1};
...
for i = 1:numTraj                                           % for all trajectories loop
    ...
    [DB,DW,DT]                          = deal(zeros([1 3 trajLen(i)]));
    [B,AB,T,AT,VB,W,AW,VW,VT]           = deal(zeros([3 3 trajLen(i)]));
    
    [L,Lt]                              = deal(zeros(N,3,trajLen(i)));
    zeta                                = zeros(N,1,trajLen(i));
    ...
    L(1:N,1:3,1) = L0;
    B(:,:,1) = eye(3);
    % tmp = tau_eta * reshape(dudx(start_points(i),:),[3 3]);           % !!! July 19, non-dimensional treatment.
    tmp = tau_eta * [dudx(start_points(i)),dudy(start_points(i)),dudz(start_points(i));...
    ...
    num_of_records = hdf('VS','write',vdata_id,{reshape(B,9,[]),reshape(PB,1,[]),reshape(QB,1,[]),reshape(RB,1,[]),reshape(DB,3,[]),reshape(VB,9,[]),...
...
% % Lt = sqrt(sum(L(1:N,1:3,:).^2,2));
% % Lt = sum(L(1:N,1:3,:).^2,2);
% % dlnLdt = diff(log(Lt),1,3);
% 
% for i = 1:length(traj)
%     traj(i).age = 0:trajLen(i)-1;
% end
% age = cat(2,traj.age);
% 
% feval('save',[hfile(1:end-4),'_LPQR'],'hfile','numPoints','numTraj','start_points','tau_eta','age','L','L0',...
%     'B','PB','QB','RB','DB','VB','zeta','AB','QAB','RAB','T','DT','VT','AT','PT','QT','RT','QAT','RAT',...
%     'W','AW','PW','QW','RW','QAW','RAW','DW','VW');
% % feval('save',[hfile(1:end-4),'_NOTNORMALIZED_LPQR'],'L','PB','QB','RB','DB','VB','zeta','hfile','numPoints','numTraj','start_points');
% 
% %%%%%%%%%%%%%%% Figures %%%%%%%%%%%%%%%%%%%
% 
% plot_lls_v7;
