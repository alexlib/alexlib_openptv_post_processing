...
% This is kind of hard coded,no letter/description after Exp    
%     direc2=['F:\Exp_09112010\Exp',num2str(m),'_09112010','\image'];
%     direc=['F:\Exp_09112010\Exp',num2str(m),'_09112010','\res'];
%     d = [direc,'/ptv_is.1*'];
   

% This is flexible,anything begins with Exp is read from there on. (including b or multi) 
    mm=['F:\Exp_09112010'];
    kkk=[mm,'\Exp*'];
    kkkk=dir(kkk);
    si=size(kkkk,1);
    for tt=1:si
        file=[mm,'\',kkkk(tt).name];
    ...
    for n=first:last
        if mod(n,5)==0
            ...
            sprintf('Processing: %s ,file =%d, out of =%d',file,n,num)
        ...
        name_ptv=[direc,'\ptv_is.',num2Str(n)];
        ...
        fidd = fopen([f1,'\ptv_is_v2.',num2Str(n)],'w');
        fprintf(fidd,'%d\n',length(past));
        fprintf(fidd,'%4d %4d %9.4f %9.4f %9.4f %9.4f %9.4f %9.4f %9.4f %9.4f %9.4f %9.4f %9.4f %9.4f\n',[past,future,x_m,y_m,z_m,total_pix,xpix,ypix,grv,strain,lambda1,lambda2,lambda3,velocity]');
        fclose(fidd);
