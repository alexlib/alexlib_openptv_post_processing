hfile =  'F:\Matlab\Alex\HDF\0104_hompol.hdf'
...
%'reldiv',[0 0.1]);
% [traj,attr] = readTrajHDF_v9(hfile,'u',[],'v',[],'w',[],'w1',[],'w2',[],'w3',[],...
%     's11',[],'s12',[],'s13',[],'s22',[],'s23',[],'s33',[],'minlength',20);
...
% 
% Re_lambda =   mean(sqrt(uSq)) * Taylor_lambda / 1.e-6
...
data = hdfread([hfile(1:end-4),'_lpqr.hdf'],hfile(1:end-4),'Fields','d,v','firstrecord',1);
...
mean(squeeze(D(:,3,:)))
% 
% mean(squeeze(D(:,1,inddiv)))
% mean(squeeze(D(:,2,inddiv)))
% mean(squeeze(D(:,3,inddiv)))

% feval('save',[hfile(1:end-4),'_eigens.mat'],'D','V' ,'trajLen','numPoints','numTraj','inddiv');
...
% saveas(hf,['pdf_lambda_sumlambda2_',hfile(1:end-4)],'fig')% Plot histogram of the eigenvalues
% % saveas(hf,['pdf_lambda_sumlambda2_',hfile(1:end-4)],'fig')% Plot histogram of the eigenvalues
% 
% 
% % --------------------------------------------------------------------
...
%     %     cos_sS(j) = cosine(s(:)',S);
%     % cos_sS(j) =
%     % sum(s(:)'.*S,2)./sqrt(sum(s(:)'.^2,2))./sqrt(sum(S.^2,2));
%     cos_sS(j) = sum(s(:)'.*S,2)./norm(s(:))./norm(S); 
% end
% 
% % cos_sS = cosine(s,S); PDF of the cos(s,S)
% hf = figure
% nhist(cos_sS,50,'b:o');
...
data = hdfread([hfile(1:end-4),'_lpqr.hdf'],hfile(1:end-4),'Fields','age','firstrecord',1);
age = data{1};
...
saveas(hf,['pdf_coswlambda1_vs_t',hfile(1:end-4)],'fig')
...
legend('\tau_{\eta} \leq 1','1 \leq \tau_{\eta} \leq 2','2 \leq \tau_{\eta} \leq 4','4 \leq \tau_{\eta} \leq 6','6 \leq \tau_{\eta}')
xlabel('cos(\omega,\lambda)')
saveas(hf,['pdf_coswlambda2_vs_t',hfile(1:end-4)],'fig')
...
legend('\tau_{\eta} \leq 1','1 \leq \tau_{\eta} \leq 2','2 \leq \tau_{\eta} \leq 4','4 \leq \tau_{\eta} \leq 6','6 \leq \tau_{\eta}')
xlabel('cos(\omega,\lambda)')
saveas(hf,['pdf_coswlambda3_vs_t',hfile(1:end-4)],'fig')
