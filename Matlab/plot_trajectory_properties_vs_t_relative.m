...
for i = 1:length(traj)
    if length(traj(i).xf) > 5
        ...
        plot(traj(i).t(2)/dt,(-1)*(traj(i).xf(2)/10-x),'r.')
        plot(traj(i).t(2)/dt,(-1)*(traj(i).yf(2)/10-y),'g.');
     

       l1=plot(traj(i).t(3:end-1)/dt,(-1)*(traj(i).xf(3:end-1)/10-x),'r--','DisplayName',num2str(i));
       l2=plot(traj(i).t(3:end-1)/dt,(-1)*(traj(i).yf(3:end-1)/10-y),'g--','DisplayName',num2str(i));
        ...
        plot(traj(i).t(end)/dt,(-1)*(traj(i).xf(end)/10-x),'r*');
...
for i = 1:length(traj)
    if length(traj(i).xf) > 5
        plot(traj(i).t(3)/dt,(-1)*(traj(i).uf(3)*dt/10),'r.');
        plot(traj(i).t(3)/dt,(-1)*(traj(i).vf(3)*dt/10),'g.');
       
      %  plot(traj(i).t(3)/dt,traj(i).wf(3)*dt/10,'b.');

        l1=plot(traj(i).t(3:end-2)/dt,(-1)*(traj(i).uf(3:end-2)*dt/10),'r-','DisplayName',num2str(i));
        l2=plot(traj(i).t(3:end-2)/dt,(-1)*(traj(i).vf(3:end-2)*dt/10),'g--','DisplayName',num2str(i));
      %  l3=plot(traj(i).t(3:end-2)/dt,traj(i).wf(3:end-2)*dt/10,'b-.','DisplayName',num2str(i));

        plot(traj(i).t(end-2)/dt,(-1)*(traj(i).uf(end-2)*dt/10),'r*');
        plot(traj(i).t(end-2)/dt,(-1)*(traj(i).vf(end-2)*dt/10),'g*');
...
for i = 1:length(traj)
    if length(traj(i).xf) > 5
        plot(traj(i).t(3)/dt,(-1)*(traj(i).axf(3)*dt^2/10),'r.');
        plot(traj(i).t(3)/dt,(-1)*(traj(i).ayf(3)*dt^2/10),'g.');
% %         plot(traj(i).t(3)/dt,traj(i).azf(3)/10,'b.')*dt^2;
% % 
         l1=plot(traj(i).t(3:end-2)/dt,(-1)*(traj(i).axf(3:end-2)*dt^2/10),'r-','DisplayName',num2str(i));
         l2=plot(traj(i).t(3:end-2)/dt,(-1)*(traj(i).ayf(3:end-2)*dt^2/10),'g--','DisplayName',num2str(i));
% %         l3=plot(traj(i).t(3:end-2)/dt,traj(i).azf(3:end-2)*dt^2/10,'b-.','DisplayName',num2str(i));
% % 
          plot(traj(i).t(end-2)/dt,(-1)*(traj(i).axf(end-2)*dt^2/10),'r*');
          plot(traj(i).t(end-2)/dt,(-1)*(traj(i).ayf(end-2)*dt^2/10),'g*');
