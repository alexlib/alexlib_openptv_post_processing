function checks_beat_hdf(hfile,first,last,tail,stepSize)
...
hfile1 = ['../HDF/0104_hompol.hdf'];
hfile2 = ['../HDF/0104_water.hdf'];
...
for i=first:stepSize:last
    ...
    if s(1,1)>0
        ...
        for iii=1:s(1,1)-1
            ...
            if ende<s(1,1)
                for ii=start:ende
                    if age(ii)>tail && age(ii)<leng-tail+1
                        ...
                        if reldiv(ii)<0.2
                            ...
                            if good<nPoints+1
                                %                                     forMeanDiss(good,1)=strain;
                                
                                forRMSax(good,1)=ax(ii)^2;
                                forRMSay(good,1)=ay(ii)^2;
                                forRMSaz(good,1)=az(ii)^2;
                                forPDFax(good,1)=ax(ii)/rmsax;
                                forPDFay(good,1)=ay(ii)/rmsay;
                                forPDFaz(good,1)=az(ii)/rmsaz;
                                forERRax(good,1)=abs(ax(ii)-alx(ii)-acx(ii))/(abs(ax(ii))+abs(alx(ii))+abs(acx(ii)));
                                forERRay(good,1)=abs(ay(ii)-aly(ii)-acy(ii))/(abs(ay(ii))+abs(aly(ii))+abs(acy(ii)));
                                forERRaz(good,1)=abs(az(ii)-alz(ii)-acz(ii))/(abs(az(ii))+abs(alz(ii))+abs(acz(ii)));
