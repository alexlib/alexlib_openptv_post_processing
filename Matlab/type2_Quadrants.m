function [data_large,data_small] = type2_Quadrants(quantity_large,quantity_small,large,small,time_interval,close_enough,verbose)
% TYPE2 should solve the problem of complex selections of events and the
% respective flow fields.
% [DATA_LARGE,DATA_SMALL] = ...
%   TYPE2_QUADRANTS(QUANTITY_LARGE,QUANTITY_SMALL,LARGE,SMALL,TIME_INTERVAL,CLOSE_ENOUGH,VERBOSE)
% Inputs:
%   quantity_large - one of the quantities, e.g. 'xf','uf', etc.
%   quantity_small = name of the Quadrants component,
%   e.g. 'q1','q2','q3' or 'q4';
...
%   >> load large_10680
%   >> load small_10680
%   >> [data_large,data_small] = type2_Quadrants('q2','vf',large_10680,small_10680,10,20)
%
...
error('Not implemented => Quadrants of what? uv, uw or vw?')
...
for i = 1:numTraj,
    ...
    if ~isempty(event),
        ...
        data_large = cat(1,data_large,large(i).(quantity_large)(event));
        ...
        for ii = 1:numSmallTraj % for all small ones
            ...
            if ~isempty(relevant)
                ...
                if any(distances <= close_enough)
                    ...
                    % data = cat(1,data,tmp.(quantity_small)(distances <= close_enough));
                    
                    switch quantity_small
                        case 'q1'
                            data = cat(1,data,tmp.uf(distances <= close_enough).*tmp.vf(distances <= close_enough));
                        case 'q2'
                            data = cat(1,data,tmp.uf(distances <= close_enough).*tmp.wf(distances <= close_enough));
                        case 'q3'
                            data = cat(1,data,tmp.vf(distances <= close_enough).*tmp.wf(distances <= close_enough));
                        case 'q4'
                            data = cat(1,data,tmp.vf(distances <= close_enough).*tmp.wf(distances <= close_enough));
                        otherwise
                            error('Reynolds stress component is not recognized');
                    end
