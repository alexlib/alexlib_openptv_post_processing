...
%         
        if s(1,1)>0
            ...
            aBx=0.5*f1(:,45);%acx-aLamx;%%%%%%%%%%%%%%%%%%%%%
            aBy=0.5*f1(:,46);%acy-aLamy;%%%%%%%%%%%%%%%%%%%%%%%
            aBz=0.5*f1(:,47);%acz-aLamz;%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            cux=unx.*f1(:,36)+uny.*f1(:,37)+unz.*f1(:,38);%%%(u.*ux+v.*uy+w.*uz)./(absu.^2);
            cuy=unx.*f1(:,39)+uny.*f1(:,40)+unz.*f1(:,41);%%%(u.*vx+v.*vy+w.*vz)./(absu.^2);
            cuz=unx.*f1(:,42)+uny.*f1(:,43)+unz.*f1(:,44);%%%(u.*wx+v.*wy+w.*wz)./(absu.^2);
