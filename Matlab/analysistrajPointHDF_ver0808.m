hfile =  'F:\Matlab\Alex\HDF\0104_water.hdf'
...
hfile = 'E:\Rotating\Matlab_codes\script_graveyard\run2.hdf'
[traj,attr] = readTrajHDF_v9(hfile,'x',[],'y',[],'z',[],'u',[],'v',[],'w',[],'t',[],'minlength',20);

%%
tmp = zeros(length(traj),1); for i=1:length(traj), tmp(i) = std(traj(i).y.^2); end
ids = find(tmp > 50 & tmp < 200)
close all
figure, hold on
for i=1:length(ids), plot3(traj(ids(i)).x, traj(ids(i)).y, traj(ids(i)).z); end
