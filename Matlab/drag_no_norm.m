...
if ismac
    matdirectory = '/Users/alex/Dropbox/resuspension/2011/trajectories(186-194)/events_110';
else
...
%ws = density_s * pi * 4 / 3 * (r*100)^3 ;%submerged particle weight [g]
%fv = (1+0.5*(density_l/density_s - density_l)); % 
%f_drag_critical_1 = fv * ws /1000 ; 
        ...
        for ind = 1:length(data.t)
            ...
            for j = 1:numSmallTraj % for all small ones
                ...
                if ~isempty(relevant)
                    ...
                    drag_force{kFiles,iEvents,j} =  0.5* density_l*1000* CD * A * norm(velocity_p_f) * velocity_p_f;% sum of all the relevant small particles
            ...
            data_drag{kFiles,iEvents,ind} = drag_force ; % removed the norm
...
for kFiles = 1:size(data_drag,1)
    for iEvents = 1:size(data_drag,2)
        if ~isempty([time_of_event{kFiles,iEvents}])
            ...
            for i = 1:size(tmp,2) % for all small particles
                plot([time_data_small{kFiles,iEvents,i}]-[time_data_small{kFiles,iEvents,1}],tmp(i)); 
            end
